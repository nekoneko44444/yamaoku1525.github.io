...
let avatar,viewMode='follow',demoRoute=[],demoIndex=0,demo=false,renderedAt=0;
...
function setSpawn(id){
  ...
  clearInput();position={x:s.x,z:s.z,floor:world.floor(s.x,s.z).height};yaw=s.yaw;pitch=0;eye=position.floor+world.data.eyeHeight;sim=0;last=0;dirty=true;avatar?.reset(position,yaw);updateCamera();drawMap();
}
function updateCamera(){
  if(viewMode==='eyes'){
    ...
    const desired=new THREE.Vector3(position.x-Math.sin(yaw)*2.05,position.floor+1.43-pitch*.75,position.z+Math.cos(yaw)*2.05);
...
function resize(){if(!renderer)return;const w=stage.clientWidth,h=stage.clientHeight;renderer.setPixelRatio(light?1:Math.min(devicePixelRatio,1.5));renderer.setSize(w,h,false);camera.aspect=w/h;camera.fov=w<h?76:66;camera.updateProjectionMatrix();dirty=true;}
function drawMap(){
...
function tick(time){
  ...
  while(sim>=1/60){
    ...
    const speed=(keys.has('ShiftLeft')||keys.has('ShiftRight'))?.45:.78;
    ...
    if(demo&&demoRoute.length){
      ...
      if(demo){const step=Math.min(.60/60,d);v.dx=dx/d*step;v.dz=dz/d*step;const targetYaw=Math.atan2(dx,-dz);yaw+=Math.atan2(Math.sin(targetYaw-yaw),Math.cos(targetYaw-yaw))*.10;}
...
$('view-mode').onclick=()=>{viewMode=viewMode==='follow'?'eyes':'follow';$('view-mode').textContent=viewMode==='follow'?'自分の目線にする':'なぎかと歩く';dirty=true;stage.focus({preventScroll:true});};

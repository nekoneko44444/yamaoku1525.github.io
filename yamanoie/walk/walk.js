...
import {WalkWorld,movementVector} from './movement.mjs?v=8d656ab19f02';
import {Nagika} from './avatar.js?v=8d656ab19f02';
...
async function start(){
  ...
  try{
    dispose();lost=false;
    const res=await fetch('./navigation.json?v=8d656ab19f02');if(!res.ok)throw new Error('歩行データを取得できません');world=new WalkWorld(await res.json());
    ...
    const manifestResponse=await fetch('./nagika-manifest.json?v=8d656ab19f02');if(!manifestResponse.ok)throw new Error('なぎかのモデル情報を読み込めません');const manifest=await manifestResponse.json();
    ...
    const routeResponse=await fetch('./demo-route.json?v=8d656ab19f02');if(routeResponse.ok)demoRoute=(await routeResponse.json()).points;
